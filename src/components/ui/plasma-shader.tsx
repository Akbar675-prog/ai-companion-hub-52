...
/** Animated WebGL1 "Plasma" shader canvas — fills its positioned parent. */
export function PlasmaShader({ className }: { className?: string }) {
  ...
  React.useEffect(() => {
    ...
    gl.uniform3fv(u("u_colors[0]"), new Float32Array(COLORS.flat().concat(new Array(12).fill(0))));
    gl.uniform4f(u("u_shape"), 1.5, 0.48, 0.5, 0.0);
    gl.uniform4f(u("u_surface"), 2.4, 0.92, -0.5, 1.0);
    gl.uniform4f(u("u_finish"), 3.04, 0.61, 0.016, 0.35);
    gl.uniform4f(u("u_transform"), 7.0, 0.0, 0.16, 0.0);
    gl.uniform4f(u("u_space"), 0, 0, 0, 0);
    gl.uniform4f(u("u_cursor"), 0, 4.0, 0.65, 0.3);
    ...
    function frame(now: number) {
      resize();
      gl!.uniform4f(sceneLoc, canvas!.width, canvas!.height, ((now - start) / 1000) * 0.86, 4.0);
...
/** Circular AI avatar powered by the plasma shader. */
export function AiAvatar({ className }: { className?: string }) {
  return (
    <span
      className={`relative inline-block overflow-hidden rounded-full bg-[#160B0B] ring-1 ring-black/10 ${className ?? "size-7"}`}
      aria-hidden
    >
      <PlasmaShader />
    </span>
